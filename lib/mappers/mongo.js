...
function MongoMapper (dbUrl) {
	...
	this.find = Bluebird.method(function (ModelType, query) {
		if (!(ModelType.prototype instanceof Model)) {
			throw new Error("Model type does not inherit from Model");
		}

		return getCollection(collectionName(ModelType), collectionIndex(ModelType))
		.then(function (collection) {
		...
		.map(function (record) {
			return scrub(ModelType, record);
		});
	});

	this.findOne = Bluebird.method(function (ModelType, query) {
		if (!(ModelType.prototype instanceof Model)) {
			throw new Error("Model type does not inherit from Model");
		}
