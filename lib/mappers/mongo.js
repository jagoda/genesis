...
function MongoMapper () {
	var collectionPool = {};
	var connection = MongoClient.connectAsync(MongoMapper.DEFAULT_URL);
	...
	function getCollection (collection, index) {
		...
		return new Bluebird(function (resolve) {
			if (collectionPool[collection]) {
				resolve(collectionPool[collection]);
			}
			else {
				resolve(connection
					...
					.then(function (dbCollection) {
						collectionPool[collection] = dbCollection;
						return dbCollection;
					})
				);
			}
